...
namespace OptitrackManagement
{
	...
	public static class DirectMulticastSocketClient {
        ...
        //private static string _multicastIPAddress = "127.0.0.1";
		...
		private static void StartClient() 
		{
			...
			try
			{
				...
				Console.WriteLine("[UDP] Starting client");
			...
			{
                Console.WriteLine("[UDP] DirectMulticastSocketClient: " + e.ToString());
			}
		}
		...
		private static bool Receive(Socket client) 
		{
			try 
			{
                ...
                Console.WriteLine("[UDP multicast] Receive");
		...
		private static void ReceiveCallback( IAsyncResult ar ) 
		{
			try {
				...
				{
                    Console.WriteLine("[UDP] - End ReceiveCallback");
					
					if(_isIsActiveThread == false)
					{
                        Console.WriteLine("[UDP] - Closing port");
		...
		private static void ReadPacket(Byte[] b)
		{
			...
			{
                Console.WriteLine("DirectParseClient: Data descriptions");
			...
			}else if (messageID == 7)   // Frame of Mocap Data
			{
                ...
                /***************** EDIT START **********************/
                long frameIndex = iData[0];
                //Console.WriteLine(iData[1].ToString());     // THIS GETS THE FRAME ID - maybe just assume fs = 100 Hz
                //float[] pleaseGod = new float[100];
                //Buffer.BlockCopy(b, (offset - 4), pleaseGod, 0, 4);
                //Console.WriteLine(pleaseGod[0].ToString());
				...
				{
					ReadRigidBody(b, ref offset, _dataStream._rigidBody[i], frameIndex);
				}   
